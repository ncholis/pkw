...
from master.models import Pengunjung
import json
...
def set_dataset_actual():
  data_actual = Pengunjung.objects.get_queryset().dataset_actual()
  return {
        'label': 'Demand / Actual',
        'fill': 'false',
        'borderWidth': 2,
...
def set_dataset_forecast():
  data_forecast = Pengunjung.objects.get_queryset().dataset_forecast()
  return {
       'label': 'Forecast 3',
       'fill': 'false',
       'borderWidth': 2,
...
def set_dataset_absolute():
  data_absolute = [abs(int(x)) for x in set_dataset_error()['data']]
  return {
       'label': 'Absolute',
       'fill': 'false',
       'borderWidth': 2,
...
def set_dataset_error_percent():
  data_absoulute = [ pow(int(x), 2) for x in set_dataset_absolute()['data']]
  data_actual = Pengunjung.objects.get_queryset().dataset_actual()

  results = []
  for x in range(len(data_actual)):
    r = (data_absoulute[x] / data_actual[x]) * 100
    results.append(r)
  ...
  return {
       ...
       'pointHoverRadius': 7,
       'pointColor': ' #f5b7b1 ',
       'pointBackgroundColor': ' #f5b7b1 ',
       'data': results
    }


def set_dataset_squared():
  data_squared = [ pow(int(x), 2) for x in set_dataset_error()['data']]
  return {
       'label': 'Error',
       'fill': 'false',
       'borderWidth': 2,
...
def set_dataset_error():
  data_error = Pengunjung.objects.get_queryset().dataset_error()
  return {
       'label': 'Error',
       'fill': 'false',
       'borderWidth': 2,
...
def set_prediction_next_month():
  d = Pengunjung.objects.get_queryset().prediction_next_month()
  return d
...
def index(request):
  #template chart
  dataset_date = Pengunjung.objects.get_queryset().dataset_date()
  chart = {
    'labels': dataset_date,
    'datasets': [
      set_dataset_actual(),
      set_dataset_forecast(),
      set_dataset_error(),
      set_dataset_absolute(),
      set_dataset_squared(),
      set_dataset_error_percent()
    ],
  }

  ttl = Pengunjung.objects.aggregate(Sum('jumlah'))['jumlah__sum']
  if not ttl:
    ttl = 0
  ...
  context = {
    ...
    'segment': 'dashboardv1',
    'ttl': ttl,
    'salesChartData': json.dumps(chart),
    'prediction_next_month': set_prediction_next_month(),
  }
